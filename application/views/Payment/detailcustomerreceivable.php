		...
		<div class="panel mt-6">
			...
			<div class="px-5">
				...
				<div class="grid grid-cols-1 gap-3 sm:grid-cols-4">
					<?php foreach ($data['get_customer_data'] as $row) { ?>
						<div>
							<label for="customer_name">Nama customer:</label>
							<input id="customer_name" type="text" id="customer_name" class="form-input" readonly value="<?= $row->customer_name ?>">
						</div>
						
						<div>
							<label for="receivable_nominal">Total Piutang:</label>
							<input id="receivable_nominal" type="text" class="form-input" readonly value="<?= 'Rp '. number_format($row->customer_receivable_total) ?>">
						</div>

                        <div>
							<label for="total_nota">Total Nota:</label>
							<input id="total_nota" type="text" class="form-input" readonly value="<?= number_format($row->customer_receivable_count) ?>">
						</div>
		...
		<div class="px-5">
			...
			<div class="flex items-center justify-between mb-3">
				...
				<div class="flex items-center gap-2">
                                        ...
                                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                                            <form class="space-y-5">
                                                <div>
                                                    ...
                                                    <input id="receivable_customer_id_add" type="hidden" class="form-input"/>
                                                    <input id="receivable_invoice_add" type="text" class="form-input" placeholder="Masukan No Invoice Penjualan"/>
                                                </div>
                                                <div>
                                                    <label for="receivable_customer_name_add" style="font-size: 13px;">Nama customer</label>
                                                    <input id="receivable_customer_name_add" type="text" class="form-input" placeholder="Masukan Nama customer" readonly/>
                                                </div>
                                                <div>
                    ...
                    <!-- END MODAL ADD -->
                         <!-- MODAL EDIT -->
                    <div class="fixed inset-0 z-[999] hidden bg-black/60" :class="$store.paymentreceivable.openEdit && '!block'">
                        <div class="flex min-h-screen items-center justify-center px-4">
                            <div  class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >

                                <div class="flex justify-between px-5 py-3 border-b">
                                    <h5 class="text-lg font-bold">Pembayaran Piutang</h5>
                                    <button @click="$store.paymentreceivable.openEdit = false">✕</button>
                                </div>
                                <div class="p-5 space-y-4">
                                    <div>
                                        <label>customer</label>
                                        <input type="text" x-model="$store.paymentreceivable.form.customer_name" class="form-input" id="customer_name_payment" readonly/>
                                    </div>
                                    <div>
                                        <label>Nota</label>
                                        <input type="hidden" x-model="$store.paymentreceivable.form.receivable_id" class="form-input" id="receivable_id_payment" readonly/>
                                        <input type="text" x-model="$store.paymentreceivable.form.nota" class="form-input" id="nota_payment" readonly/>
                                    </div>
                                    <div>
                                        <label>Piutang</label>
                                        <input type="text" x-model="$store.paymentreceivable.form.receivable_amount" class="form-input" id="receivable_amount_payment" value="0" readonly/>
                                    </div>
                                    <div>
                                        <label>Nominal Bayar</label>
                                        <input type="text" x-model="$store.paymentreceivable.form.payment_amount" class="form-input" id="payment_amount_payment" value="0"/>
                                    </div>
                                    <div>
                                        <label>Sisa Piutang</label>
                                        <input type="text" x-model="$store.paymentreceivable.form.remaining_receivable" class="form-input" id="remaining_receivable_new" value="0" readonly/>
                                    </div>
                                    <div class="flex items-center rounded bg-danger-light p-3.5 text-danger dark:bg-danger-dark-light  error-message">
                                        <span class="notif-box">
                                            <strong class="title">Warning!</strong>
                                ...
                                <div class="flex justify-end gap-2 p-5 border-t">
                                    <button class="btn btn-outline-danger" @click="$store.paymentreceivable.openEdit = false">Cancel</button>
                                    <button class="btn btn-primary" id="save-paymentreceivable">Bayar</button>
                                </div>
...
<script>
    ...
    let receivable_amount_payment_val = new AutoNumeric('#receivable_amount_payment', {
        currencySymbol : 'Rp. ',
        decimalCharacter : ',',
        decimalPlaces: 0,
    ...
    let remaining_receivable_new_val = new AutoNumeric('#remaining_receivable_new', {
        currencySymbol : 'Rp. ',
        decimalCharacter : ',',
        decimalPlaces: 0,
        digitGroupSeparator : '.',
    ...
    window.Payment = function(id, nominal, nota) {
        ...
        Alpine.store('paymentreceivable').form.customer_name = '';
        Alpine.store('paymentreceivable').form.receivable_id = '';
        receivable_amount_payment_val.set(0);
        payment_amount_payment_val.set(0);
        remaining_receivable_new_val.set(0);
        Alpine.store('paymentreceivable').form.nota = '';

        Alpine.store('paymentreceivable').openEdit = true;
        Alpine.nextTick(() => {
            Alpine.store('paymentreceivable').form.customer_name = customerName;
            Alpine.store('paymentreceivable').form.receivable_id = id;
            receivable_amount_payment_val.set(nominal);
            remaining_receivable_new_val.set(nominal);
            Alpine.store('paymentreceivable').form.nota = nota;
        });
    }
    ...
    try{
        document.getElementById('payment_amount_payment').addEventListener('input', function(){
            ...
            const receivable = receivable_amount_payment_val.getNumber() || 0;
            const remaining = Math.max(0, receivable - paid);
            remaining_receivable_new_val.set(remaining);
        });
    }catch(e){}

	document.addEventListener('alpine:init', () => {
		// initialize paymentreceivable store to be used by window.Payment and modals
		Alpine.store('paymentreceivable', {
			openEdit: false,
			form: {
				receivable_id: '',
				receivable_amount: '',
	...
	document.addEventListener('alpine:init', () => {
		Alpine.data('multicolumn', () => ({
			...
			async loadData() {
				...
				try {
					// get customer id from querystring (set by link)
					let customerId = '<?= isset($_GET['id']) ? $_GET['id'] : '' ?>';

					let response = await fetch("<?= base_url('Payment/get_receivable_detail') ?>", {
						method: "POST",
						headers: {
					...
					let tableData = res.data.map((item, index) => [
						index + 1,
                        item.customer_receivable_invoice,
                        'Rp ' + item.customer_receivable_nominal,
                        'Rp ' + item.customer_receivable_remaining,
						'',
						item.customer_id,
                        item.customer_receivable_id,
                        item.customer_receivable_remaining_no_format
						]);
					this.datatable = new simpleDatatables.DataTable('#myTable', {
						data: {
							headings: ['No', 'No Invoice', 'Total Piutang', 'Sisa Piutang', 'Aksi', 'ID', 'receivable Id', 'Last Piutang'],
							data: tableData
						},
						searchable: true,
						perPage: 10,
						...
						columns: [
						{
							...
							render: (data, cell, row) => {
                                ...
                                <a href="javascript:;" x-tooltip="Bayar Piutang" class="mr-2" onclick="window.Payment('${row.cells[6].data}', '${row.cells[7].data}', '${row.cells[1].data}')">
                                ...
                                <!-- DELETE (kanan) -->
                                 <a href="javascript:;" x-tooltip="Delete" onclick="window.deletereceivable('${row.cells[6].data}')">
    ...
    $('#save-paymentreceivable').on('click', function (e) {
        e.preventDefault();

        let receivable_id_payment             = $("#receivable_id_payment").val();
        let nota_payment                = $("#nota_payment").val();
        let payment_amount_payment      = payment_amount_payment_val.get();
        let remaining_receivable_new          = remaining_receivable_new_val.get();
        let receivable_amount_payment         = receivable_amount_payment_val.get();

        $.ajax({
            type: "POST",
            url: "<?= base_url('Payment/save_payment_receivable') ?>",
            dataType: "json",
            data: {
                receivable_id_payment: receivable_id_payment,
                nota_payment: nota_payment,
                payment_amount_payment: payment_amount_payment,
                remaining_receivable_new: remaining_receivable_new,
                receivable_amount_payment: receivable_amount_payment
            },
            success: function (data) {
                if (data.code == "200") {
                    showNotif('success', data.message);
...
window.deletereceivable = function(id, useSwal = true) {

    const doDelete = () => {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Payment/delete_receivable') ?>",
            dataType: "json",
...
<script>
// set receivable_customer_id_add from querystring if available
try{
    document.getElementById('receivable_customer_id_add').value = '<?= isset($_GET['id']) ? $_GET['id'] : '' ?>';
    // also set customer name for add modal from header customer_name field if present
    try{
        var customerNameHeader = document.getElementById('customer_name');
        if(customerNameHeader && document.getElementById('receivable_customer_name_add')){
            document.getElementById('receivable_customer_name_add').value = customerNameHeader.value || '';
        }
    }catch(e){}
}catch(e){}
</script>