                                        ...
                                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                                            <form class="space-y-5">
                                                ...
                                                <div>
                                                    ...
                                                    <select id="debt_supplier_id_add" class="form-input">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('multicolumn', () => ({
            ...
            async loadData() {
                ...
                try {
                    let response = await fetch("<?= base_url('Payment/get_debt') ?>", {
                        method: "POST",
                        headers: {
                    ...
                    let tableData = res.data.map((item, index) => [
                        ...
                        item.supplier_name,
                        item.supplier_phone,
                        item.supplier_debt_total,
                        item.supplier_debt_count,
                        '',
                        item.supplier_id
                        ]);
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['No', 'Kode Supplier', 'Nama Supplier', 'Telp', 'Total Hutang', 'Jumlah Nota', 'Aksi', 'ID'],
                            data: tableData
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [
                        {
                            ...
                            render: (data, cell, row) => {
                                ...
                                <a href="<?= base_url('Payment/detailsupplierdebt?id=') ?>${row.cells[7].data}" x-tooltip="List" class="mr-2" onclick="window.editCustomer('${row.cells[7].data}', '${row.cells[1].data}', '${row.cells[2].data}', '${row.cells[3].data}', '${row.cells[4].data}')">
...
// initialize Select2 on supplier select (if select2 is loaded)
try{
    (function(){
        function initSelect2(){
            try{
                if(typeof $ === 'undefined') return console.warn('jQuery missing for select2');
                var $sel = $('#debt_supplier_id_add');
                if(!$sel || $sel.length === 0) return console.warn('select element #debt_supplier_id_add not found');
                var $parent = $sel.closest('.fixed.inset-0');
                if(!$parent || $parent.length === 0) $parent = $(document.body);
                $sel.select2({
                ...
                console.log('Select2 initialized for #debt_supplier_id_add');
            ...
            success: function (data) {
                if (data.code == "200") {
                    ...
                    try{
                        ...
                        } else if(document.getElementById('debt_supplier_id_add')){
                            document.getElementById('debt_supplier_id_add').value = '';
                        }
                        $('.error-message').hide();
                    }catch(e){console.warn('clear modal error', e)}
