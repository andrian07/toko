        ...
        <div class="px-5">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-2">
                            ...
                            <div class="flex min-h-screen items-start justify-center px-4">
                                <div class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >
                                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                                        <h5 class="text-lg font-bold">Tambah Pelanggan</h5>
                                    ...
                                    <div class="p-5">
                                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                                            <form class="space-y-5">
                                                <div>
                                                    <label for="customer_code" style="font-size: 13px;">Kode Pelanggan</label>
                                                    <input id="customer_code" type="text" class="form-input" value="AUTO" readonly/>
                                                </div>
                                                <div>
                                                    <label for="customer_name" style="font-size: 13px;">Nama Pelanggan</label>
                                                    <input id="customer_name" type="text" class="form-input" placeholder="Masukan Nama Pelanggan" />
                                                </div>
                                                <div>
                                                    <label for="customer_phone" style="font-size: 13px;">No Telp</label>
                                                    <input id="customer_phone" type="text" class="form-input" placeholder="Masukan No Telp Pelanggan" />
                                                </div>
                                                <div>
                                                    <label for="customer_address" style="font-size: 13px;">Alamat</label>
                                                    <textarea id="customer_address" class="form-input" placeholder="Masukan Alamat Pelanggan"></textarea>
                                                </div>  
                                            </form>
                                        ...
                                        <div class="mt-8 flex items-center justify-end">
                                            ...
                                            <button type="button" id="save-customer" class="btn btn-primary ltr:ml-4 rtl:mr-4">Save</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <!-- MODAL EDIT -->
                    <div class="fixed inset-0 z-[999] hidden bg-black/60" :class="$store.customer.openEdit && '!block'">
                        <div class="flex min-h-screen items-center justify-center px-4">
                            <div  class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >

                                <div class="flex justify-between px-5 py-3 border-b">
                                    <h5 class="text-lg font-bold">Edit Pelanggan</h5>
                                    <button @click="$store.customer.openEdit = false">✕</button>
                                </div>
                                <div class="p-5 space-y-4">
                                    <div>
                                        <label>Kode Pelanggan</label>
                                        <input type="hidden" x-model="$store.customer.form.customer_id" class="form-input" id="customer_id_edit">
                                        <input type="text" x-model="$store.customer.form.customer_code" class="form-input" id="customer_code_edit" readonly/>
                                    </div>
                                    <div>
                                        <label>Nama Pelanggan</label>
                                        <input type="text" x-model="$store.customer.form.customer_name" class="form-input" id="customer_name_edit" />
                                    </div>
                                    <div>
                                        <label>No Telp</label>
                                        <input type="text" x-model="$store.customer.form.customer_phone" class="form-input" id="customer_phone_edit" />
                                    </div>
                                    <div>
                                        <label>Alamat</label>
                                        <textarea x-model="$store.customer.form.customer_address" class="form-input" id="customer_address_edit"></textarea>
                                    </div>
                                    <div class="flex items-center rounded bg-danger-light p-3.5 text-danger dark:bg-danger-dark-light  error-message">
                                        <span class="notif-box">
                                            <strong class="title">Warning!</strong>
                                ...
                                <div class="flex justify-end gap-2 p-5 border-t">
                                    <button class="btn btn-outline-danger" @click="$store.customer.openEdit = false">Cancel</button>
                                    <button class="btn btn-primary" id="edit-customer">Edit</button>
                                </div>
...
<script>
    ...
    window.editCustomer = function(id, code, name, phone, address) {
        console.log(code);
        Alpine.store('customer').form.customer_id = id;
        Alpine.store('customer').form.customer_code = code;
        Alpine.store('customer').form.customer_name = name;
        Alpine.store('customer').form.customer_phone = phone;
        Alpine.store('customer').form.customer_address = address;

        Alpine.store('customer').openEdit = true;
        Alpine.nextTick(() => {
            Alpine.store('customer').form.customer_id = id;
            Alpine.store('customer').form.customer_code = code;
            Alpine.store('customer').form.customer_name = name;
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('multicolumn', () => ({
            ...
            async loadData() {
                ...
                try {
                    let response = await fetch("<?= base_url('Masterdata/get_customer') ?>", {
                        method: "POST",
                        headers: {
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['No', 'Kode Pelanggan', 'Nama Pelanggan', 'Telp', 'Alamat', 'Piutang', 'Aksi', 'ID'],
                            data: tableData
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [
                        {
                            ...
                            render: (data, cell, row) => {
                                ...
                                <a href="javascript:;" x-tooltip="Edit" class="mr-2" onclick="window.editCustomer('${row.cells[7].data}', '${row.cells[1].data}', '${row.cells[2].data}', '${row.cells[3].data}', '${row.cells[4].data}')">
                                ...
                                <!-- DELETE (kanan) -->
                                <a href="javascript:;" x-tooltip="Delete" onclick="window.deleteCustomer('${row.cells[7].data}')">
...
function clearCustomerForm(){
    try{
        ...
        $('#customer_code').val('AUTO');
        $('.error-message').hide();
        $('.message-content').text('');
        $('#customer_name, #customer_phone, #customer_address').removeClass('input-error');
    }catch(e){ }
}
...
window.deleteCustomer = function(id, useSwal = true) {

    const doDelete = () => {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Masterdata/delete_customer') ?>",
            dataType: "json",
            data: { customer_id: id },
            success: function (data) {
                if (data.code == "200") {
