        ...
        <div class="px-5">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-2">
                            ...
                            <div class="flex min-h-screen items-start justify-center px-4">
                                <div class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >
                                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                                        <h5 class="text-lg font-bold">Tambah Kategori</h5>
                                    ...
                                    <div class="p-5">
                                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                                            <form class="space-y-5">
                                                <div>
                                                    <label for="category_code" style="font-size: 13px;">Kode Kategori</label>
                                                    <input id="category_code" type="text" class="form-input" placeholder="Masukan Kode Kategori" />
                                                    
                                                </div>
                                                <div>
                                                    <label for="category_name" style="font-size: 13px;">Nama Kategori</label>
                                                    <input id="category_name" type="text" class="form-input" placeholder="Masukan Nama Kategori" />
                    ...
                    <!-- MODAL EDIT -->
                    <div class="fixed inset-0 z-[999] hidden bg-black/60" :class="$store.category.openEdit && '!block'">
                        <div class="flex min-h-screen items-center justify-center px-4">
                            <div  class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >

                                <div class="flex justify-between px-5 py-3 border-b">
                                    <h5 class="text-lg font-bold">Edit Kategori</h5>
                                    <button @click="$store.category.openEdit = false">✕</button>
                                </div>
                                <div class="p-5 space-y-4">
                                    <div>
                                        <label>Kode Kategori</label>
                                        <input type="hidden" x-model="$store.category.form.category_id" class="form-input" id="category_id_edit">
                                        <input type="text" x-model="$store.category.form.category_code" class="form-input" id="category_code_edit">
                                    </div>
                                    <div>
                                        <label>Nama Kategori</label>
                                        <input type="text" x-model="$store.category.form.category_name" class="form-input" id="category_name_edit">
                                    </div>
                                    <div class="flex items-center rounded bg-danger-light p-3.5 text-danger dark:bg-danger-dark-light  error-message">
                                        <span class="notif-box">
                                            <strong class="title">Warning!</strong>
                                ...
                                <div class="flex justify-end gap-2 p-5 border-t">
                                    <button class="btn btn-outline-danger" @click="$store.category.openEdit = false">Cancel</button>
                                    <button class="btn btn-primary" id="edit-category">Edit</button>
                                </div>
...
<script>
    ...
    window.editCategory = function(id, code, name) {
        Alpine.store('category').form.category_id = id;
        Alpine.store('category').form.category_code = code;
        Alpine.store('category').form.category_name = name;
        Alpine.store('category').openEdit = true;
    }
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('multicolumn', () => ({
            ...
            async loadData() {
                ...
                try {
                    let response = await fetch("<?= base_url('utility/get_category') ?>", {
                        method: "POST",
                        headers: {
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['No', 'Kode Kategori', 'Nama Kategori', 'Action', 'ID'],
                            data: tableData
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [
                        {
                            ...
                            render: (data, cell, row) => {
                                ...
                                <a href="javascript:;" x-tooltip="Edit" class="mr-2" onclick="window.editCategory('${row.cells[4].data}', '${row.cells[1].data}', '${row.cells[2].data}')">
                                ...
                                <!-- DELETE (kanan) -->
                                <a href="javascript:;" x-tooltip="Delete" onclick="window.deleteCategory('${row.cells[4].data}')">
...
$('#save-category').on('click', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "<?= base_url('Utility/save_category') ?>",
        dataType: "json",
        data: {
...
$('#edit-category').on('click', function (e) {
    ...
    var category_code = $("#category_code_edit").val();
    var category_name = $("#category_name_edit").val();

    $.ajax({
        type: "POST",
        url: "<?= base_url('Utility/edit_category') ?>",
        dataType: "json",
        data: {
        ...
        success: function (data) {
             if (data.code == "200") {
                showNotif('success', data.message);
                Alpine.store('category').openEdit = false;
                window.tableInstance.reloadTable();
            } else {
                $('.error-message').show();
                $('.message-content').text(data.message);
...
window.deleteCategory = function(id, useSwal = true) {

    const doDelete = () => {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Utility/delete_category') ?>",
            dataType: "json",
            data: { category_id: id },
            success: function (data) {
                if (data.code == "200") {
