        ...
        <div class="px-5">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="flex items-center gap-2">
                            ...
                            <div class="flex min-h-screen items-start justify-center px-4">
                                <div class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >
                                    <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                                        <h5 class="text-lg font-bold">Tambah Unit</h5>
                                    ...
                                    <div class="p-5">
                                        <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                                            <form class="space-y-5">
                                                <div>
                                                    <label for="unit_code" style="font-size: 13px;">Kode Unit</label>
                                                    <input id="unit_code" type="text" class="form-input" placeholder="Masukan Kode Unit" />
                                                    
                                                </div>
                                                <div>
                                                    <label for="unit_name" style="font-size: 13px;">Nama Unit</label>
                                                    <input id="unit_name" type="text" class="form-input" placeholder="Masukan Nama Unit" />
                                        ...
                                        <div class="mt-8 flex items-center justify-end">
                                            ...
                                            <button type="button" id="save-unit" class="btn btn-primary ltr:ml-4 rtl:mr-4">Save</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <!-- MODAL EDIT -->
                    <div class="fixed inset-0 z-[999] hidden bg-black/60" :class="$store.unit.openEdit && '!block'">
                        <div class="flex min-h-screen items-center justify-center px-4">
                            <div  class="panel animate__animated animate__slideInDown my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0" >

                                <div class="flex justify-between px-5 py-3 border-b">
                                    <h5 class="text-lg font-bold">Edit Unit</h5>
                                    <button @click="$store.unit.openEdit = false">✕</button>
                                </div>
                                <div class="p-5 space-y-4">
                                    <div>
                                        <label>Kode Unit</label>
                                        <input type="hidden" x-model="$store.unit.form.unit_id" class="form-input" id="unit_id_edit">
                                        <input type="text" x-model="$store.unit.form.unit_code" class="form-input" id="unit_code_edit">
                                    </div>
                                    <div>
                                        <label>Nama Unit</label>
                                        <input type="text" x-model="$store.unit.form.unit_name" class="form-input" id="unit_name_edit">
                                    </div>
                                    <div class="flex items-center rounded bg-danger-light p-3.5 text-danger dark:bg-danger-dark-light  error-message">
                                        <span class="notif-box">
                                            <strong class="title">Warning!</strong>
                                ...
                                <div class="flex justify-end gap-2 p-5 border-t">
                                    <button class="btn btn-outline-danger" @click="$store.unit.openEdit = false">Cancel</button>
                                    <button class="btn btn-primary" id="edit-unit">Edit</button>
                                </div>
...
<script>
    ...
    window.editUnit = function(id, code, name) {
        Alpine.store('unit').form.unit_id = id;
        Alpine.store('unit').form.unit_code = code;
        Alpine.store('unit').form.unit_name = name;
        Alpine.store('unit').openEdit = true;
    }
        ...
        Alpine.data('modal', () => ({
            ...
            toggle() {
                ...
                if (this.open) {
                    ...
                    $('#unit_code, #unit_name').removeClass('input-error');
                }
            }
        }));
    });
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('multicolumn', () => ({
            ...
            async loadData() {
                ...
                try {
                    let response = await fetch("<?= base_url('utility/get_unit') ?>", {
                        method: "POST",
                        headers: {
                    ...
                    this.datatable = new simpleDatatables.DataTable('#myTable', {
                        data: {
                            headings: ['No', 'Kode Unit', 'Nama Unit', 'Action', 'ID'],
                            data: tableData
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [
                        {
                            ...
                            render: (data, cell, row) => {
                                ...
                                <a href="javascript:;" x-tooltip="Edit" class="mr-2" onclick="window.editUnit('${row.cells[4].data}', '${row.cells[1].data}', '${row.cells[2].data}')">
                                ...
                                <!-- DELETE (kanan) -->
                                <a href="javascript:;" x-tooltip="Delete" onclick="window.deleteUnit('${row.cells[4].data}')">
...
$('#save-unit').on('click', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "<?= base_url('Utility/save_unit') ?>",
        dataType: "json",
        data: {
            unit_code: unit_code,
            unit_name: unit_name
...
$('#edit-unit').on('click', function (e) {
    ...
    var unit_code = $("#unit_code_edit").val();
    var unit_name = $("#unit_name_edit").val();

    $.ajax({
        type: "POST",
        url: "<?= base_url('Utility/edit_unit') ?>",
        dataType: "json",
        data: {
            unit_id: unit_id,
            unit_code: unit_code,
        ...
        success: function (data) {
             if (data.code == "200") {
                showNotif('success', data.message);
                Alpine.store('unit').openEdit = false;
                window.tableInstance.reloadTable();
            } else {
                $('.error-message').show();
                $('.message-content').text(data.message);
...
window.deleteUnit = function(id, useSwal = true) {

    const doDelete = () => {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Utility/delete_unit') ?>",
            dataType: "json",
            data: { unit_id: id },
            success: function (data) {
                if (data.code == "200") {
